...
export default function Accomps() {
  const router = useRouter();

  // Exemple de données pour le tableau avec des identifiants uniques
  const enfantsData = [
    { id: '1', famille: 'SAANY Loka', nbre: '5' },
    { id: '2', famille: 'LISABI Akala', nbre: '2' },
  ];
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.header}>
        <View>
          <Text style={styles.title}>Mode accompagnateur</Text>
          <Text style={styles.subtitle}>Gérer mes accomps</Text>
        </View>
        <TouchableOpacity 
      ...
      <View style={styles.tableContainer}>
        <View style={styles.tableHeader}>
          ...
          <Text style={[styles.tableHeaderText, styles.columnMatiere]}>Nombre d'enfants</Text>
        ...
        {enfantsData.map((enfant, index) => (
          <View key={enfant.id} style={styles.tableRow}>
            ...
            <Text style={[styles.tableCell, styles.columnMatiere]}>{enfant.nbre}</Text>
            <TouchableOpacity 
              style={[styles.detailsButton, styles.columnDetails]}
              onPress={() => router.push(`/AccompFolder/Historiques`)} 
            >
